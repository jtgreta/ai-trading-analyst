"""
skill_market_bias.py — Institutional Edition
Pre-session market bias briefing. Run this BEFORE the Scanner every session.

Usage:
  python skill_market_bias.py           → full briefing
  python skill_market_bias.py --quiet   → one-line verdict only
...
def print_bias(b: dict):
    ...
    if b["verdict"] == "🟢 RISK ON":
        print(f"\n  → Run scanner next: python skill_scanner_run.py")
