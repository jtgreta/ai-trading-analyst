"""Hunter — institutional pre-breakout coil scanner with SMC overlay.

Usage: ``python hunter.py``

Pipeline: tickers → filter → 15m/1h/4h SMC scoring → trade plans (score >= 6)
→ watch list (score 4-5) → verdict + Telegram.

SMC additions over a plain range screen:
  * BOS/ChoCh context for breakout direction confirmation
  * FVG detection — is the breakout target an unfilled imbalance?
  * Order Block detection — breakout into a supply/demand zone
  * ATR-based stop sizing, ChoCh bonus, inducement + OB confluence (+3)
...
def score_coin(cand: dict, k15: list, k1h: list, k4h: list) -> dict:
    """Score one candidate: range compression, volume, structure, SMC context."""
    score = 0
    direction = "SKIP"
    notes = []
    ...
    if len(closed_15) >= 8:
        ...
        if avg_prev > 0:
            ...
            if ratio < 0.50:
                score += 3
                notes.append(f"tight squeeze ({ratio:.2f}x range)")
            elif ratio < 0.65:
                score += 2
                notes.append(f"moderate compression ({ratio:.2f}x range)")
            elif ratio < 0.80:
                score += 1
                notes.append(f"mild compression ({ratio:.2f}x range)")
    ...
    if len(closed_15) >= 8:
        ...
        if prev4_v > 0:
            ...
            if vr > 1.4:
                score += 2
                notes.append(f"volume building ({vr:.2f}x)")
            elif vr > 1.1:
                score += 1
                notes.append(f"volume slightly up ({vr:.2f}x)")
            else:
                notes.append(f"volume flat ({vr:.2f}x)")
    ...
    if ms_4h["trend"] == "bullish" and ms_1h["trend"] in ("bullish", "ranging"):
        score += 2
        direction = "LONG"
        notes.append("4H bullish structure + 1H aligned")
    elif ms_4h["trend"] == "bearish" and ms_1h["trend"] in ("bearish", "ranging"):
        score += 2
        direction = "SHORT"
        notes.append("4H bearish structure + 1H aligned")
    elif ms_4h["trend"] == "bullish":
        score += 1
        direction = "LONG"
        notes.append("4H bullish (1H conflicting)")
    elif ms_4h["trend"] == "bearish":
        score += 1
        direction = "SHORT"
        notes.append("4H bearish (1H conflicting)")
    else:
        # Fallback to MA stack when structure is ranging.
        cl_1h = [c[4] for c in k1h]
        cl_4h = [c[4] for c in k4h]
        ma10_1h = sma(cl_1h, 10)
        ma20_1h = sma(cl_1h, 20)
        ma10_4h = sma(cl_4h, 10)
        ma20_4h = sma(cl_4h, 20)
        ...
        if bull_1h or bull_4h:
            score += 1
            direction = "LONG"
            notes.append("MA lean bullish (ranging structure)")
        elif bear_1h or bear_4h:
            score += 1
            direction = "SHORT"
            notes.append("MA lean bearish (ranging structure)")
    ...
    # Order blocks (confluence for inducement check).
    ...
    idms = identify_inducement(
        k1h, ms_1h["swing_highs"], ms_1h["swing_lows"], ms_1h["trend"], lookback=30
    )
    idm_flag = ""
    ...
    if rel_vol > 1.5:
        score += 1
        notes.append(f"rel vol {rel_vol:.2f}x (above avg)")
    else:
    ...
    if rsi_val:
        if direction == "LONG" and rsi_val > 75:
            score -= 1
            rsi_flag = f"⚠️  RSI {rsi_val:.0f} overbought — late entry risk"
        elif direction == "SHORT" and rsi_val < 25:
            score -= 1
            rsi_flag = f"⚠️  RSI {rsi_val:.0f} oversold — counter-trend risk"
...
def build_trade_plan(cand: dict, sig: dict, k1h: list) -> dict:
    """ATR-based trade plan for a qualified coil setup.

    ATR is pre-computed in :func:`score_coin` from ``k1h``; the actual 1H candle
    list passed in is kept for contract compatibility.
    """
    symbol = cand["symbol"]
    price = cand["lastPrice"]
    ...
    # Entry zone from the last 4 closed 15M candles.
    c15 = sig["closed_15"]
    entry_low = min(c[3] for c in c15[-4:])
    entry_high = max(c[2] for c in c15[-4:])
    entry_mid = (entry_low + entry_high) / 2

    # ATR-aware stop anchored to the entry midpoint.
    if sig["atr_val"] is not None:
        sl_dist = min(sig["atr_val"] * 1.5, entry_mid * rules["max_sl"])
    else:
        # Fallback: entry-zone range as a volatility proxy.
    ...
    # Refine TP1 to FVG boundary when in breakout direction with >= 1.5 RR.
    if sig["nearby_fvg"]:
        fvg = sig["nearby_fvg"]
...
def print_setup(i: int, cand: dict, sig: dict, plan: dict) -> None:
    symbol = cand["symbol"]
    score = sig["score"]
    direction = sig["direction"]
...
Type       : {plan['label']}  |  Grade: {plan['grade']} ({"Full" if plan['grade']=='A' else "Half" if plan['grade']=='B' else "25%"} size)
...
def build_telegram_msg(qualified: list, watch_list: list, verdict_line: str, now: str) -> str:
    lines = [
        "🎯 HUNTER — PRE-BREAKOUT SCAN",
        f"{now}",
        "Vol >$100M | Range 3–25% | SMC Engine\n",
    ]
    if qualified:
...
def main() -> None:
    ...
    print(f"\n{'═'*60}")
    print("  🎯 HUNTER — PRE-BREAKOUT SCAN  (SMC Edition)")
    print(f"  {now} PHT")
    print(f"{'═'*60}")
    ...
    candidates, _ = hunter_candidates(tickers)
    ...
    if qualified:
        ...
        reason = (
            "No coins passed filter — low liquidity window"
            if not candidates
