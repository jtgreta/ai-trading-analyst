"""Scalper — institutional multi-timeframe scalp protocol (deep per-symbol chart).

Usage: ``python scalper.py SYMBOL [open1,open2,...]``
...
Hard rules (never violated):
  ...
  * Min RR 1:2 (TP1 >= 2x SL distance)
  * Max risk $3-$4  |  Score < 6 = WAITING ROOM
  * WAITING ROOM always fires when no valid trade

Top-down flow: 4H structure (macro gate) → 1H structure (ChoCh) → 15m OTE
(precise entry) → 5m execution (final confirmation).
"""
...
def _telegram_structure(ms_4h: dict, ms_1h: dict) -> dict:
    return {
        "tf_4h": f"4H {ms_4h['trend'].upper()}",
        "tf_1h": f"1H {ms_1h['trend'].upper()}",
        "event": ms_4h["last_event"] or "—",
    }


def analyze_scalp(symbol: str, open_positions: list = None) -> None:
    ...
    idms = identify_inducement(
        k1h, ms_1h["swing_highs"], ms_1h["swing_lows"], ms_1h["trend"], lookback=40
    )
    ...
    # Moving averages
    def mas(closes):
        return {p: sma(closes, p) for p in [5, 10, 20, 30, 60]}
    ...
    # HF-SESSION: low-vol window favorable to Grade A setups.
    ...
    if ms_4h["trend"] == "bullish" and (ms_1h["trend"] == "bullish" or ms_1h["last_event"] == "BOS_up"):
        score += 4
        notes.append("4H bullish + 1H aligned ✓")
    elif ms_4h["trend"] == "bearish" and (ms_1h["trend"] == "bearish" or ms_1h["last_event"] == "BOS_down"):
        score += 4
        notes.append("4H bearish + 1H aligned ✓")
    elif ms_4h["trend"] == "bullish":
        score += 2
        notes.append("4H bullish (1H not yet confirmed — partial)")
    elif ms_4h["trend"] == "bearish":
        score += 2
        notes.append("4H bearish (1H not yet confirmed — partial)")
    elif ms_4h["last_event"] in ("BOS_up", "ChoCh_up"):
        score += 2
        notes.append(f"4H structural shift bullish ({ms_4h['last_event']}) — forming")
    elif ms_4h["last_event"] in ("BOS_down", "ChoCh_down"):
        score += 2
        notes.append(f"4H structural shift bearish ({ms_4h['last_event']}) — forming")

    # ChoCh deduction on 1H
    if ms_1h["mss"]:
        score -= 1
        notes.append(f"⚠️ ChoCh on 1H ({ms_1h['last_event']}) — structure reversing")
    ...
    if ote and ote["ote_bottom"] <= price <= ote["ote_top"]:
        score += 2
        notes.append("In Optimal Trade Entry (OTE) zone")
        ote_valid = True

    # Inducement / liquidity grab (max +1)
    if idms:
        score += 1
        notes.append(f"Recent Liquidity Grab / Inducement ({idms[0]['type']})")
    ...
    if len(vol_1h) >= 6:
        ...
        if avg_vol_5 > 0 and vol_1h[-1] > avg_vol_5 * 1.5:
            score += 1
            notes.append("1H volume spike (>1.5× avg) ✓")

    # Order book bias (max +1)
    if bid_pct > 53:
        score += 1
        notes.append(f"Order book long-biased ({bid_pct:.0f}% bids) ✓")
    elif (100 - bid_pct) > 53:
        score += 1
        notes.append(f"Order book short-biased ({100 - bid_pct:.0f}% asks) ✓")
    ...
    if go_waiting:
        ...
        trigger = (
            fmt(ms_1h.get("swing_high", price))
            if ms_4h["trend"] != "bearish"
            else fmt(ms_1h.get("swing_low", price))
        )
    ...
    if soft_warnings:
        print(f"\n{'─'*60}")
        print("  ⚠️  WARNINGS — Trade not blocked, proceed with caution:")
    ...
    # Stop calculation anchored to the refined entry.
    stops = calc_atr_stop(
        k1h, entry, direction,
        multiplier=1.5, period=14, max_sl_pct=rules["max_sl"],
    )
    ...
    print_trade_plan(
        ...
        stops=stops,
        sizing=sizing,
        trail_callback_pct=trail_pct,
        invalidation=stops["sl"],
        tf_label="4H",
    )

    # ── SECTION 4: TELEGRAM (TRADE PLAN) — always sent ────────────────────────
    tg_msg = format_telegram_trade(
        symbol=symbol,
        direction=direction,
        grade=grade,
        session=session,
        structure=_telegram_structure(ms_4h, ms_1h),
        zones={"entry": entry},
        risk={
            "sl": stops["sl"],
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scalper.py SYMBOL [open1,open2,...]")
        sys.exit(1)
