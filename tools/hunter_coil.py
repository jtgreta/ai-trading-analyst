...
Purpose:
  ...
  For a full market-wide scan, use hunter.py instead.
...
from trading import (
    ...
    print_market_structure, print_trade_plan, print_waiting_room,
...
def build_trade_plan(symbol: str, price: float, quote_vol: float,
    ...
    SL = ATR x 1.5 from entry midpoint, capped at token max_sl%.
    TP1 refined to FVG top if FVG is in breakout direction and RR >= 1.5.

    Args:
        k1h: raw 1H candle list — kept for ATR context; ATR is pulled from sig.
    """
    ...
    # ATR-based stop, falling back to the coil-zone range when ATR is missing.
    if sig["atr_val"]:
...
def print_coil_analysis(symbol: str, price: float, quote_vol: float, sig: dict, k1h: list):
    ...
    print_market_structure(
        sig["ms_4h"], sig["ms_1h"], "4H", "1H",
        sig["rsi_val"], fvg_nearest, ob_nearest, liq_eqh, liq_eql,
    )
    ...
    if score >= 6 and direction != "SKIP":
        ...
        print_trade_plan(
            ...
            entry_note="Breakout trigger / 15M range mid",
            stops={
                "sl": plan["sl"],
                "sl_pct": plan["sl_pct"] / 100,
                "tp1": plan["tp1"],
                "tp2": plan["tp2"],
            },
            sizing={
                "label": plan["type_label"],
                "lev": plan["lev"],
                "margin": plan["margin"],
                "risk": plan["actual_risk"],
            },
    ...
    preconditions = {
        "Range Contraction (15M)": sig["score"] >= 4,
...
def build_telegram(results: list, session: dict, now: str) -> str:
    ...
    if qualified:
        ...
        for sym, sig, plan in qualified[:3]:
            ...
            fvg = (
                f" | FVG:{fmt(sig['nearby_fvg']['bottom'])}-{fmt(sig['nearby_fvg']['top'])}"
    ...
    lines += ["", f"⏰ {session['window']} | {session['volatility']}"]
    msg = "\n".join(lines)
