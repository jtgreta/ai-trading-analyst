"""Scanner — Institutional Binance Futures market-wide SMC screen.

Full pipeline: filter → klines → SMC flags → priority scoring → ranked output.
Companion deep-analysis skill: ``scalper`` (per-symbol trade plan).

Key gates (unchanged from the original engine):
  * BOS/ChoCh structure detection replaces pure MA-stack direction
  * FVG proximity flag, Order Block proximity flag
  * ATR volatility filter (skip if ATR > 4% of price)
  * RSI extreme filter (1H >78 / <22)
  * Hard filter: no direction assigned when 4H structure is ranging (MA fallback)
  * Inducement (liquidity grab) detection, OTE confluence check
"""
...
def analyze_coin(coin: dict, k1h: list, k4h: list, k15m: list) -> dict:
    """Run the full SMC diagnostic for a single candidate."""
    sym = coin["symbol"]
    ...
    # OTE zone confluence on the current 1H structure.
    ote_zone = False
    ote_flag = ""
    ...
    vol_signal = (
        "Spike" if last_vol > avg5_vol * 1.5
        else "Fading" if last_vol < avg5_vol * 0.7
        else "Normal"
    )

    # Relative volume vs 24H hourly baseline.
    ...
    rsi_warn = (
        ...
        if rsi_val and rsi_val > RSI_EXTREME_HIGH
        else f"RSI {rsi_val:.0f} OVERSOLD"
        if rsi_val and rsi_val < RSI_EXTREME_LOW
        else None
    )
    ...
    # Coiling: 15M range contraction AND volume building (both must hold).
    ranges_15 = [c[2] - c[3] for c in k15m]
    vols_15 = [c[5] for c in k15m]
    last3r, prev3r = ranges_15[-3:], ranges_15[-6:-3]
    last3v, prev3v = vols_15[-3:], vols_15[-6:-3]
    coiling = (
        (sum(last3r) / 3 < sum(prev3r) / 3 * 0.75) if prev3r else False
    ) and (
        (sum(last3v) / 3 > sum(prev3v) / 3 * 1.2) if prev3v else False
    )
    ...
    if idm_flag:
        why = "Price recently swept internal liquidity (inducement trap) and reversed"
    elif ote_flag:
        why = "Price is in Optimal Trade Entry (OTE) discount zone"
    elif nearby_fvg:
        ...
        why = "BOS on 1H — trend continuation breakout confirmed"
    elif coiling:
    ...
    if "⏳" not in direction:
        pscore += 1
    if ms_4h["last_event"] and "BOS" in ms_4h["last_event"]:
        pscore += 4
    if nearby_fvg:
        pscore += 3
    if idms:
    ...
    if ote_zone:
        pscore += 2
    if ms_1h["last_event"] and "BOS" in ms_1h["last_event"]:
        pscore += 1
    if nearby_ob:
        pscore += 1
    if ms_1h["mss"]:
...
def main() -> None:
    ...
    print(f"\n{'═'*60}")
    print("  ⚡ BINANCE FUTURES SCANNER  (Institutional SMC Edition)")
    print(f"{'═'*60}")
    ...
    filtered_coins, total_matched = scanner_candidates(tickers)
    ...
    for coin in filtered_coins:
        ...
        if not k1h or not k4h or not k15m:
            continue
        if len(k1h) < 30 or len(k4h) < 20:
            continue
    ...
    if picks:
        for i, res in enumerate(picks, 1):
            dir_short = (
                "LONG" if "LONG" in res["direction"]
                else "SHORT" if "SHORT" in res["direction"]
                else "SKIP"
            )
        ...
        if not session.get("can_enter", True):
    ...
    if picks:
        ...
        tg_lines.append(
            f"Reason: {session['advice'] if not session['can_enter'] else 'No setup confluence'}"
        )
