"""
journal.py — Institutional Edition
Trade logger + weekly review + compliance tracker.

Usage:
  python journal.py log SYMBOL DIRECTION ENTRY SL TP1 TP2 GRADE SCORE OUTCOME [NOTE]
  python journal.py review             → weekly review summary
  python journal.py stats              → all-time stats
  python journal.py list [N]           → last N trades (default 10)
  python journal.py delete ID          → remove a trade entry
...
Examples:
  python journal.py log SOLUSDT LONG 145.50 140.00 156.00 165.00 B 7 win_tp1
  python journal.py log ETHUSDT SHORT 3200 3320 3080 2970 A 9 loss_sl "news spike"
  python journal.py review
  python journal.py stats

Storage: Trades are saved to journal.json in the repo root.
Purpose: Enables the mandatory weekly review from docs/trading_rules.md.
...
def main():
    ...
    if command == "log":
        ...
        print("""
journal.py — Trade Logger & Review Tool
...
Examples:
  python journal.py log SOLUSDT LONG 145.5 140 156 165 B 7 win_tp1
  python journal.py log BTCUSDT SHORT 65000 66000 63000 61500 A 9 loss_sl "news spike"
  python journal.py list 20
  python journal.py review
  python journal.py stats
""")
