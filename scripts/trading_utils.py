...
def _load_config():
    """Load optional repo-root config/trading.json overlay (capital, risk, rules)."""
    cfg_path = os.path.join(_repo_root(), "config", "trading.json")
...
if CONFIG.get("max_risk"):
    MAX_RISK = float(CONFIG["max_risk"])
if CONFIG.get("min_rr"):
...
def _apply_rule_overrides(dst, src):
    if not isinstance(src, dict):
        return
    for tok, override in src.items():
        if tok in dst and isinstance(override, dict):
            for k, v in override.items():
                if v is not None:
                    dst[tok][k] = v

_apply_rule_overrides(SCALP_RULES, CONFIG.get("scalp_rules"))
_apply_rule_overrides(SWING_RULES, CONFIG.get("swing_rules"))
