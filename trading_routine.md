...
| :--- | :--- | :--- | :--- |
| **2:45 PM** | **Review Open Positions** | `skill_manage.py SYM DIR E SL TP1 TP2` | Check open swing positions. Get live P&L and management advice. |
| **3:00 PM** | **Market Bias Check** | `skill_market_bias.py` | Get RISK ON / RISK OFF / NEUTRAL verdict **before** scanning. |
| **3:05 PM** | **Funding Check** | `skill_funding.py` | Flag coins with extreme funding. Avoid those directions. |
| **3:10 PM** | **Run Scanner** | `skill_scanner_run.py` | Identify top movers and SMC-ranked coins for the session. |
| **3:20 PM** | **Deep Analysis** | `skill_scalper_analyze.py X` | If a **Grade A (Score >= 8)** setup exists → run Scalper. |
| **5:00 PM** | **London Fade** | — | No new entries unless a perfect Swing setup appears. |
| **9:00 PM** | **Re-Run Bias** | `skill_market_bias.py` | Confirm bias hasn't flipped since London open. |
| **9:05 PM** | **Re-Run Scanner** | `skill_scanner_run.py` | Market rotates. Check for new priority coins. |
| **9:15 PM** | **Final Analysis** | `skill_scalper_analyze.py X` | Run Scalper on top 1–2 NY picks. |
| **9:30 PM** | **Execution** | — | Enter if **Score >= 6** and all checklist items are checked. |
| **11:30 PM** | **Trade Management** | `skill_manage.py` | Manage open positions. Be extremely selective about new entries. |
| **12:00 AM** | **Hard Stop** | — | **No new entries** regardless of setup quality. |
| **Post-trade** | **Log Every Trade** | `skill_journal.py log ...` | Record outcome immediately — win, loss, or BE. |
| **Weekend** | **Weekly Review** | `skill_journal.py review` | Mandatory compliance + performance review. |
...
```
python skill_journal.py review   → compliance, win rate, expectancy, violations
python skill_journal.py stats    → all-time performance
```
