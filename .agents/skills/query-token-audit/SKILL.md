...
- If `hasResult=false` OR `isSupported=false`:
  â†’ Reply: "Security audit data is not available for this token on this chain."
  â†’ Do NOT show `riskLevel`, `riskLevelEnum`, or `riskItems` (data is unreliable when either field is false)
  â†’ You may suggest the user verify the contract address and chain, or try again later
- If `hasResult=true` AND `isSupported=true`:
  â†’ Show the full audit result including risk level, tax info, and all risk items
  â†’ Apply the Risk Level Reference table above for actionable guidance
...
7. Always end with disclaimer: `âš ï¸ This audit result is for reference only and does not constitute investment advice. Always conduct your own research.`
